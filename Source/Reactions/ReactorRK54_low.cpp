...
#include "ReactorRK54_low.H"

namespace pele::physics::reactions {

int
ReactorRK54::init(int reactor_type, int /*ncells*/)
{
  BL_PROFILE("Pele::ReactorRK54::init()");
  ...
  pp.query("rk54_nsubsteps_guess", rk54_nsubsteps_guess);
  pp.query("rk54_nsubsteps_min", rk54_nsubsteps_min);
  pp.query("rk54_nsubsteps_max", rk54_nsubsteps_max);
...
{
  BL_PROFILE("Pele::ReactorRK54::react()");
  ...
  const int captured_nsubsteps_guess = rk54_nsubsteps_guess;
  const int captured_nsubsteps_min = rk54_nsubsteps_min;
  const int captured_nsubsteps_max = rk54_nsubsteps_max;
  ...
  amrex::ParallelFor(ncells, [=] AMREX_GPU_DEVICE(int icell) noexcept {
    ...
    while (current_time < time_out) {
      ...
      for (int stage = 0; stage < rkp.nstages_rk54; stage++) {
        utils::fKernelSpec<Ordering>(
          0, 1, current_time - time_init, captured_reactor_type, soln_reg, ydot,
          rhoe_init, rhoesrc_ext, rYsrc_ext, leosparm);

        for (int sp = 0; sp < neq; sp++) {
          error_reg[sp] += rkp.err_rk54[stage] * dt_rk * ydot[sp];
          soln_reg[sp] =
            carryover_reg[sp] + rkp.alpha_rk54[stage] * dt_rk * ydot[sp];
          carryover_reg[sp] =
            soln_reg[sp] + rkp.beta_rk54[stage] * dt_rk * ydot[sp];
        }
      }
      ...
      if (max_err < captured_abstol) {
        change_factor =
          rkp.betaerr_rk54 * pow((captured_abstol / max_err), rkp.exp1_rk54);
        dt_rk = amrex::min<amrex::Real>(dt_rk_max, dt_rk * change_factor);
      } else {
        change_factor =
          rkp.betaerr_rk54 * pow((captured_abstol / max_err), rkp.exp2_rk54);
...
)
{
  ...
  const int captured_nsubsteps_max = rk54_nsubsteps_max;
  const amrex::Real captured_abstol = absTol;
  const amrex::Real captured_reltol = relTol;
  const auto* leosparm = m_d_eosparm;
  RK54Params rkp;
  ...
  amrex::ParallelFor(box, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
    ...
    while (current_time < time_out) {
      ...
      // BL_PROFILE_VAR("MySolver::ReactorRK54::cF_RHS()",rhsFunc);
      for (int stage = 0; stage < rkp.nstages_rk54; stage++) {
        utils::fKernelSpec<Ordering>(
          0, 1, current_time - time_init, captured_reactor_type, soln_reg, ydot,
          rhoe_init, rhoesrc_ext, rYsrc_ext, leosparm);
      ...
      // weighted root-meansquare (WRMS) truncation error
      amrex::Real wrms = 0.0;
      for (int sp = 0; sp < neq; sp++) {
        wrms += pow(fabs(error_reg[sp])*1.0/(captured_reltol*soln_reg[sp]+captured_abstol),2);
      }
      wrms  = std::sqrt(wrms / static_cast<amrex::Real>(neq));
      // if (max_err < captured_abstol) {
      //   change_factor =
      //     rkp.betaerr_rk54 * pow((captured_abstol / max_err), rkp.exp1_rk54);
      //   dt_rk = amrex::min<amrex::Real>(dt_rk_max, dt_rk * change_factor);
      ...
      //   dt_rk = amrex::max<amrex::Real>(dt_rk_min, dt_rk * change_factor);
      // }
      change_factor = rkp.betaerr_rk54*pow(1.0/wrms, rkp.exp1_rk54);
      dt_rk = amrex::min<amrex::Real>(dt_rk_max, dt_rk * change_factor);
      dt_rk = amrex::max<amrex::Real>(dt_rk_min, dt_rk * change_factor);
